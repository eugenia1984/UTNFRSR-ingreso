// Un profesor prepara tres cuestionarios para una evaluación final: A, B y C. 
// Se sabe que se tarda 5 minutos en revisar el cuestionario A, 
// 8 en revisar el cuestionario B y 6 minutos en el C. 
// La cantidad de examenes de cada tipo se entran por teclado
// ¿ Cuántas horas y cuántos minutos se tardará en revisar todas las evaluaciones? 
Proceso Ejercicio7
	Definir cantidadA, cantidadB, cantidadC Como Entero;
	Definir tiempoA, tiempoB, tiempoC Como Entero;
	Definir tiempoTotal, horas, minutos Como Entero;
	
	Escribir "Ingrese la cantidad de cuestionarios A: ";
	Leer cantidadA;
	Escribir "Ingrese la cantidad de cuestionarios B: ";
	Leer cantidadB;
	Escribir "Ingrese la cantidad de cuestionarios C: ";
	Leer cantidadC;
	// Calculo los minutos que se tardara por cada cuestionario
	tiempoA <- cantidadA * 5;
	tiempoB <- cantidadB  * 8 ;
	tiempoC <- cantidadC * 6;
	// Tiempo total que le toma revisar todos los cuestionarios
	tiempoTotal <- tiempoA + tiempoB + tiempoC;
	// Calculamos las horas y minutos
	horas <- trunc( tiempoTotal/60);
	minutos <- tiempoTotal MOD 60;
	
	Escribir "Tardara ", horas, " horas y ", minutos, " minutos.";
FinProceso
